<template>
  <div class="swap-popup">
    ...
    <p class="subtitle">
      Make SOLARI work for you! Stake your SOLARI and gain sSOLARI. No impermanent
      loss, no loss of governance rights. Continuously compounding. After each
      new deposit, all staked SOLARI are subject to a 24H lock-up period!
    </p>

    <p class="subtiitle">
      sSOLARI automatically earns fees from slUSD repayments from all users
...
export default {
  ...
  computed: {
    sSolariTokenObject() {
      return this.$store.getters.getSSolariObject;
    },
    fromToken() {
      if (this.action === "stake")
        return this.sSolariTokenObject.stakeToken.name;
      if (this.action === "unstake")
        return this.sSolariTokenObject.mainToken.name;
      return "";
    },
    toToken() {
      if (this.action === "stake") return this.sSolariTokenObject.mainToken.name;
      if (this.action === "unstake")
        return this.sSolariTokenObject.stakeToken.name;
      return "";
    },
  ...
  methods: {
    ...
    async getBalance() {
      let contract;
      if (this.action === "stake") {
        contract = this.sSolariTokenObject.stakeToken.contractInstance;
      }
      if (this.action === "unstake") {
        contract = this.sSolariTokenObject.mainToken.contractInstance;
      }
      if (!contract) {
    ...
    async getSolariRate() {
      const solariContract = this.sSolariTokenObject.stakeToken.contractInstance;
      const ssolariContract = this.sSolariTokenObject.mainToken.contractInstance;
      try {
        const ssolariBalance = await solariContract.balanceOf(
          ssolariContract.address
        );
        const totalSupply = await ssolariContract.totalSupply();
        const parsedBalance = this.$ethers.utils.formatEther(
          ssolariBalance.toString()
        );
    ...
    async actionHandler() {
      if (this.action === "stake") {
        const isApproved = await this.isTokenApprowed(
          this.sSolariTokenObject.stakeToken.contractInstance
        );
        if (isApproved) {
          await this.stake();
          return false;
    ...
    async stake() {
      ...
      try {
        ...
        const estimateGas = await this.sSolariTokenObject.mainToken.contractInstance.estimateGas.mint(
          amount
        );
        const gasLimit = 1000 + +estimateGas.toString();
        console.log("gasLimit:", gasLimit);
        const tx = await this.sSolariTokenObject.mainToken.contractInstance.mint(
          amount,
          {
            gasLimit,
          }
    ...
    async unstake() {
      ...
      try {
        const amount = this.$ethers.utils.parseEther(this.amount);
        console.log("AMOUNT", amount.toString());
        const estimateGas = await this.sSolariTokenObject.mainToken.contractInstance.estimateGas.burn(
          this.account,
          amount
        );
        const gasLimit = 1000 + +estimateGas.toString();
        console.log("gasLimit:", gasLimit);
        const tx = await this.sSolariTokenObject.mainToken.contractInstance.burn(
          this.account,
          amount,
          {
            gasLimit,
    ...
    async isTokenApprowed(tokenContract) {
      try {
        const addressApprowed = await tokenContract.allowance(
          this.account,
          this.sSolariTokenObject.mainToken.contractInstance.address,
          {
            gasLimit: 1000000,
          }
        );
        console.log(
          "SOLARI TOKEN APPROVE:",
          addressApprowed,
          parseFloat(addressApprowed.toString()) > 0
        );
        return parseFloat(addressApprowed.toString()) > 0;
      } catch (e) {
        console.log("SOLARI isApprowed err:", e);
        return false;
      }
    },
  ...
  async created() {
    ...
    this.tokenRate = await this.getSolariRate();
  },
  components: {
    ValueInput,
  },
